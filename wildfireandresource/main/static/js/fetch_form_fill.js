...
let url = window.location.origin + "/"; // or PROD_URL in production.
...
function create_shift_ticket(){
    ...
    fetch('/create_shift_ticket/', {
        ...
        body: JSON.stringify({project_id: project_id, desc: desc})
    })
    .then(response => {
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
...
function construct_page(page_num, slide){
    ...
    let formHTML = `<form id="myForm"><div class="${slide} form-container"> <h2>Section ${page_num}</h2>`;
        form_data.data.forEach(item => {
            if(item.Page === page_num){
                ...
                if (item.Type === "question") {
                    ...
                    formHTML += `<label for="${item.No}00" class="form-label">${item.No}. ${item.Entry}</label>`;
                    formHTML += `<select class="form-control" id="${item.No}00" aria-describedby="${item.Entry.replace(/\s+/g, '')}Help" required><option value="">Select an option</option>`
                ...
                formHTML += `<small id="${item.Entry.replace(/\s+/g, '')}Help" class="form-text text-muted">${item.Description}</small>
                </div>`;
            }
        ...
        formHTML += `<div class="d-flex justify-content-start mt-4">
                        <div class="d-flex justify-content-between mt-4 mb-4" style="width: 360px;">
            <button type="button" class="btn btn-light" id="prev-button">Previous</button>
            <button type="submit" class="btn btn-inverse-primary" id="save-button">Save</button>
...
function createPageLinks(formFillStatus, form_data, page) {
    ...
    for (let i = 1; i <= form_data.Num_Pages; i++) {
        ...
        let color = 'btn btn-inverse-success';
        let borderWidth = '2px';
        let fontWeightClass = '';
        ...
        if (formFillStatus.are_filled) {
            ...
            if (hasUnfilled) {
                color = 'btn btn-inverse-secondary';
                borderWidth = '2px';
        ...
        if (isCurrentPage) {
            pageLinks += `
                <li class="page-item active">
                    <button style="width: 100px; background-color: #379777; border: 1px solid #379777" class="page-link me-2 mr-2" onclick="pageChangeHandler(${i})">${i}</button>
                </li>`;
        } else {
            pageLinks += `
                <li class="page-item bg">
                    <button
                        style="width: 100px; border: ${borderWidth} solid ${color};"
                        class="page-link me-2 mr-2 ${fontWeightClass} ${color}"
